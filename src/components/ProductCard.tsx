...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Card className="overflow-hidden bg-white/70 glass-light border-accent group hover:border-primary/30 transition-all duration-700 shadow-premium relative h-full flex flex-col rounded-[2rem]">
      ...
      <div
        className={`absolute inset-0 z-10 bg-white/95 backdrop-blur-2xl p-10 flex flex-col justify-center transition-all duration-700 ease-in-out ${showDesc ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
          }`}
      >
      ...
      <Link href={`/catalog/${product.slug}`} className="relative aspect-square overflow-hidden shrink-0 m-4 rounded-[1.5rem] block">
        <Image
